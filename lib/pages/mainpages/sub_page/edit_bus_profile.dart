...
class _EditBusProfilePageState extends State<EditBusProfilePage> {
  ...
  Future addBusDP(String image) async {
    await FirebaseFirestore.instance.collection('passengers').where('email', isEqualTo: user?.email).get().then(
        ...
        try{
          FirebaseFirestore.instance.collection('passengers').doc(document.id).update({'bus_dp_image': image});
        } catch (e){
          print('filed');
        }
      }),
  ...
  Future addBusCover(String imagecover) async {
    await FirebaseFirestore.instance.collection('passengers').where('email', isEqualTo: user?.email).get().then(
          (snapshot) => snapshot.docs.forEach((document) {
        try{
          FirebaseFirestore.instance.collection('passengers').doc(document.id).update({'bus_cover_image': imagecover});
        } catch (e){
          print('filed');
        }
      }),
